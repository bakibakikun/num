...
log.info("Initiating payment bot system")

# Define endpoints and database connection
...
def create_payment_buttons(user_id, price):
    ...
    keyboard.add(InlineKeyboardButton("Pay with YooMoney", callback_data=f"yoomoney_{user_id}"))
    keyboard.add(InlineKeyboardButton("Pay with Crypto", callback_data=f"crypto_{user_id}"))
    keyboard.add(InlineKeyboardButton("Pay with TON Wallet", callback_data=f"ton_{user_id}"))
    return keyboard
...
for bot_key, dp in dispatchers.items():
    ...
    async def initiate_payment(msg: types.Message, bot_key=bot_key):
        try:
            ...
            log.info(f"[{bot_key}] /start by user {user_id}")
            ...
            log.info(f"[{bot_key}] Payment options sent to user {user_id}")
        except Exception as e:
            log.error(f"[{bot_key}] /start error: {e}")
            await bot_instances[bot_key].send_message(chat_id, "Error occurred. Try again.")
    ...
    async def handle_yoomoney_choice(cb: types.CallbackQuery, bot_key=bot_key):
        try:
            ...
            log.info(f"[{bot_key}] YooMoney selected by user {user_id}")
            ...
            conn.commit()
            conn.close()
            log.info(f"[{bot_key}] Saved payment: {payment_id} for user {user_id}")

            # Send payment link
            keyboard = InlineKeyboardMarkup()
            keyboard.add(InlineKeyboardButton("Complete Payment", url=payment_link))
            await bot.send_message(chat_id, "Click to pay via YooMoney:", reply_markup=keyboard)
    ...
    async def handle_crypto_choice(cb: types.CallbackQuery, bot_key=bot_key):
        try:
            ...
            bot = bot_instances[bot_key]
            await bot.answer_callback_query(cb.id)
            log.info(f"[{bot_key}] Crypto payment selected by user {user_id}")

            keyboard = InlineKeyboardMarkup()
            keyboard.add(InlineKeyboardButton("USDT", callback_data=f"crypto_usdt_{user_id}"))
            keyboard.add(InlineKeyboardButton("Bitcoin", callback_data=f"crypto_btc_{user_id}"))
            keyboard.add(InlineKeyboardButton("TON", callback_data=f"crypto_ton_{user_id}"))
            await bot.send_message(chat_id, "Select cryptocurrency:", reply_markup=keyboard)
            log.info(f"[{bot_key}] Crypto options sent to user {user_id}")
        except Exception as e:
    ...
    async def process_crypto_payment(cb: types.CallbackQuery, bot_key=bot_key):
        try:
            ...
            cfg = SETTINGS[bot_key]
            await bot.answer_callback_query(cb.id)
            log.info(f"[{bot_key}] Selected {currency} for user {user_id}")

            payment_id = str(uuid.uuid4())
            amount = cfg["PRICE"] / 80  # Approximate RUB to USD
            async with ClientSession() as session:
                ...
                async with session.post("https://api.cryptocloud.plus/v2/invoice/create", headers=headers, json=data) as resp:
                    ...
                    if result.get("status") != "success":
                        log.error(f"[{bot_key}] Crypto invoice error: {result}")
                        await bot.send_message(chat_id, "Failed to create payment. Try again.")
                        return
            ...
            conn.commit()
            conn.close()
            log.info(f"[{bot_key}] Saved crypto payment: {payment_id} for user {user_id}")
            ...
            log.info(f"[{bot_key}] Crypto payment details sent to user {user_id}")
        except Exception as e:
    ...
    async def handle_ton_payment(cb: types.CallbackQuery, bot_key=bot_key):
        try:
            ...
            cfg = SETTINGS[bot_key]
            await bot.answer_callback_query(cb.id)
            log.info(f"[{bot_key}] TON Wallet selected by user {user_id}")

            payment_id = str(uuid.uuid4())
            amount = cfg["PRICE"] / 400  # Approximate RUB to TON
            async with ClientSession() as session:
                ...
                async with session.post("https://api.tonwallet.me/v1/invoice/create", headers=headers, json=data) as resp:
                    result = await resp.json()
                    log.debug(f"[{bot_key}] TON Wallet response: {result}")
                    if result.get("status") != "success":
                        log.error(f"[{bot_key}] TON invoice error: {result}")
                        await bot.send_message(chat_id, "Failed to create TON payment. Try again.")
                        return
            ...
            conn.commit()
            conn.close()
            log.info(f"[{bot_key}] Saved TON payment: {payment_id} for user {user_id}")

            # Send payment link
            keyboard = InlineKeyboardMarkup()
            keyboard.add(InlineKeyboardButton("Pay via TON Wallet", url=pay_url))
            await bot.send_message(
                chat_id,
                f"Pay {amount:.4f} TON via Telegram Wallet:",
                reply_markup=keyboard
            )
            log.info(f"[{bot_key}] TON payment link sent to user {user_id}")
        except Exception as e:
...
async def generate_channel_invite(bot_key, user_id):
    try:
        ...
        if not bot_member.can_invite_users:
            log.error(f"[{bot_key}] Bot cannot create invites for channel {cfg['PRIVATE_CHANNEL_ID']}")
            return None
        ...
        for _ in range(3):
            try:
                ...
                log.info(f"[{bot_key}] Invite created for user {user_id}: {invite.invite_link}")
    ...
    try:
        ...
        if data.get("notification_type") in ["p2p-incoming", "card-incoming"]:
            ...
            if result:
                ...
                conn.commit()
                bot = bot_instances[bot_key]
                await bot.send_message(user_id, "Payment successful!")
                invite = await generate_channel_invite(bot_key, user_id)
                if invite:
                    await bot.send_message(user_id, f"Access channel: {invite}")
...
async def check_status(req):
    ...
    return web.Response(status=200, text=f"System active with {len(SETTINGS)} bots")
...
async def configure_webhooks():
    log.info(f"Setting webhooks for {len(SETTINGS)} bots")
    for bot_key in bot_instances:
        try:
            ...
            log.info(f"[{bot_key}] Webhook configured: {hook_url}")
        except Exception as e:
            log.error(f"[{bot_key}] Webhook error: {e}")
            sys.exit(1)
...
async def launch_server():
    try:
        ...
        log.info(f"Endpoints: {HEALTH_CHECK}, {YOOMONEY_HOOK}, {CRYPTO_HOOK}, {TON_HOOK}, {PAYMENT_STORE}, {WEBHOOK_BASE}")
        ...
        await site.start()
        log.info(f"Server active on port {port}")
